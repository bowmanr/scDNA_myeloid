...
signal_genes <- c("FLT3","NRAS","KRAS","PTPN11","JAK2")
...
list_of_mutants<-setNames(lapply(signal_genes,function(x){
...
signal_in_dominant_clone<-lapply(final_sample_summary[clonal_sample_set_after_boostrap],function(sample){
  ...
  unique(intersect(dominant_clone_genes,signal_genes))
})

list_of_mutants_in_dominant_clone<-setNames(lapply(signal_genes,function(x){
  names(signal_in_dominant_clone)[do.call(c,setNames(lapply(signal_in_dominant_clone,function(y){
    x%in%y
  }),names(signal_in_dominant_clone)))]
}),signal_genes)
...
AML<-upset(test_set%>%filter(Group=="AML"), sets=signal_genes,order.by = c("degree"),
...
tiff(width=4,height=3.75,res=300, units = "in",file="/Volumes/LevineLab/Levine Lab/Bobby/Collaborations/MissionBio/Analysis/2020/January/Figures/EF3X-upset signaling.tiff") # or other device
AML
dev.off()


NRAS_KRAS<-test_set%>%filter(Group=="AML")%>%filter(FLT3==0&NRAS==1&KRAS==1&JAK2==0&PTPN11==0&Match=="Match")%>%pull(Sample)

i <- 1
final_sample_summary[[NRAS_KRAS[i]]]$Clones
colnames(final_sample_summary[[NRAS_KRAS[i]]]$NGT)
