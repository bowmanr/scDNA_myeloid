...
# select which samples are mutated for all of the listed genes
gene_sample_selection <- function(genes){
  sample_set<- sample_mutations%>%
    select(Sample,all_of(unlist(genes)))%>%
    filter_at(vars(!Sample),all_vars(.==1)) %>%
    pull(Sample)
}

# Generate clonograph multiplot
gg_clonograph_multiplot<-function(sample){
  
  if(all(unlist(sample)%in%colnames(sample_mutations)[c(2:27,30)])){
    sample_set<- gene_sample_selection(gene=sample)
    graphs <- lapply(sample_set,gg_clonograph)
